...
BorderRadius.prototype.refresh =function(){
	var cssCode1 = '',
		cssCode2 = '';
		cssCode1+=this.topLeft+'px '+this.topRight+'px '+this.bottomRight+'px '+this.bottomLeft+'px';
		cssCode2+=this.borderWidth+'px '+this.borderStyle+' '+this.borderColor;
		//console.log("cssCode1="+cssCode1);
		//console.log("cssCode2="+cssCode2);
...
$('body').ready(function(){
	...
	$('#slider-top-right').slider({
			...
			step:1,
			slide:function(event,ui){
				var val = _getFromField(ui.value,0,200,$('#top-right'));
				borderRadius.topLeft = val;
				borderRadius.refresh();
			}
	});
	...
	$('#br-color').live('change',function(){
		...
		borderRadius.refresh();
		$('#br-color-button').css('backgound-color','#'+$(this).val());
	});
